...
variable "markitos_aws_s3_backend_env_file" {
...
variable "markitos_aws_s3_backend_s3_region" {
  description = "S3 object region where store remote backend state"
  type        = string
  nullable    = false
}

variable "markitos_aws_s3_backend_s3_bucket_encrypt" {
  description = "S3 object encrypt flag"
  type        = bool
  nullable    = false
}

variable "markitos_aws_s3_backend_s3_bucket_key" {
  description = "S3 object path/key where store remote backend state"
  type        = string
  nullable    = false
}

variable "markitos_aws_s3_backend_s3_bucket_id" {
  description = "S3 object where store remote backend state"
  type        = string
  nullable    = false
}

variable "markitos_aws_s3_backend_dynamodb_table" {
